-- Expand-compatible schema: the previous Worker can keep inserting/updating while
-- a new Worker is uploaded, and a rollback does not fail on the v2-only metadata.
CREATE TABLE leads_v4 (
  id TEXT PRIMARY KEY,
...
SELECT
  ...
  radiograph_storage_withdrawn_at, consent_version, consent_text_sha256,
  consent_captured_at, privacy_notice_url, privacy_notice_sha256,
  turnstile_hostname, deletion_token_hash, NULL, ai_model,
  pipeline_version, prompt_version, tool_schema_version, kb_version,
  private_payload
FROM leads;
