...
SELECT
  id, id, language, status, created_at, updated_at, expires_at,
  image_key, image_media_type, image_size,
  health_data_consent, radiograph_storage_consent, radiograph_storage_consent,
  NULL, consent_version, consent_text_sha256,
  consent_captured_at, privacy_notice_url, '',
  turnstile_hostname, deletion_token_hash, ai_model, private_payload
FROM leads;
...
CREATE INDEX audit_events_lead_ref_idx ON audit_events (lead_ref, occurred_at DESC);
